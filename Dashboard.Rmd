...
\
We will use the **MANN-WHITNEY U TEST** which will allow us to see if our sample of flying and groundtype pokemons' sppes come from significantly different populations, ie if the populations have a significant difference in their distribution.

**Step 3: Conduct Mann-Whitney U Test**
\
First step of the test is to rank the speed stats of both samples
```{r, echo = FALSE}
flySpeedSample
grndSpeedSample


MWtestSamplesDf = data.frame(rep(c("flying", "ground"), each = length(flySpeedSample)), c(flySpeedSample, grndSpeedSample) )
names(MWtestSamplesDf) = c("type", "speed_stat")

MWtestSamplesDf[order("speed_stat"), ]

```

Then separate the flying and ground types to get their rank sums.
```{r}
